...
void GameScreen::update(float dt) {
	...
	if (p->checkDead() && !corpseSpawned) {
		...
		if (!corpses.empty()) {
			...
			corpses.push_back(c);
			TraceLog(LOG_INFO, "Corpse spawned at %.1f %.1f, total corpses: %d", p->getPosition().x, p->getPosition().y,
			         (int)corpses.size());
		}
	}
	...
	// resolveCollisions();
	camera.target = (Vector2){p->getPosition().x + (p->getHitbox().width / 2.0f), p->getPosition().y};
	...
	if (p->isShaking()) {
		...
		int magnitude = 8.f * (t / 0.4f); // fades out over time
		if (magnitude > 0) {
